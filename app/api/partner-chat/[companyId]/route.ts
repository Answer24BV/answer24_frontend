...
export async function POST(
  ...
  try {
    ...
    const { message, history = [], user_id }: ChatRequest = await request.json();
    ...
    const settingsResponse = await fetch(`${request.nextUrl.origin}/api/widget-settings/${companyId}`);
    ...
    const relevantContext = await searchPineconeContext(message, pineconeConfig, companyId);
