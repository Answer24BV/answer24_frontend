...
import { getAvatars, deleteAvatar, type Avatar } from "@/app/[locale]/actions/avatar";
...
export default function AdminAvatarPage() {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const success = await deleteAvatar(avatarToDelete, token);
      if (success) {
        setAvatars(avatars.filter(avatar => avatar.id !== avatarToDelete));
        ...
        {isLoading ? (
          ...
          <div className={viewMode === 'grid' 
            ...
            {filteredAvatars.map((avatar) => (
              <div 
                key={avatar.id} 
                className={`group bg-white/70 backdrop-blur-sm rounded-2xl shadow-sm border border-white/20 overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-indigo-500/10 hover:-translate-y-1 ${viewMode === 'list' ? 'flex items-center p-6' : 'p-6'}`}
              >
                <div className={`flex ${viewMode === 'list' ? 'items-center space-x-6 flex-1' : 'flex-col'}`}>
                  <div className={`relative ${viewMode === 'list' ? 'flex-shrink-0' : 'mb-4 self-center'}`}>
                    ...
                    <div className={`absolute -bottom-2 -right-2 px-3 py-1 text-xs font-semibold rounded-full border-2 border-white shadow-sm ${planBadgeColors[avatar.required_plan as PlanType]}`}>
