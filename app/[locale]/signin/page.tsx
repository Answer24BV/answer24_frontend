...
import { Camera, User, Chrome } from "lucide-react"; // Import Chrome for Google icon
...
interface Errors {
  ...
  agreeTerms: string;
  profilePicture: string; // Added for profile picture validation
}

export default function SignIn() {
  const [isSignUp, setIsSignUp] = useState(false);
  const [isSignUpPartner, setIsSignUpPartner] = useState(false); // New state for partner signup
  ...
  const [errors, setErrors] = useState<Errors>({
    ...
    confirmPassword: "",
    agreeTerms: "",
    profilePicture: "", // Initialize profilePicture error
  });
  ...
  useEffect(() => {
    ...
    if (searchParams.get("partner") === "1") { // Check for partner signup
      setIsSignUp(true);
      setIsSignUpPartner(true);
    }
  }, [searchParams]);
  ...
  function validateAccountDetails(): boolean {
    ...
    const newErrors: Errors = {
      ...
      confirmPassword: "",
      agreeTerms: "",
      profilePicture: "", // Reset profilePicture error
    };

    // Validate profile picture - MADE REQUIRED HERE
  ...
  function handleProfilePictureUpload(e: ChangeEvent<HTMLInputElement>): void {
    ...
    if (file) {
      ...
      // Clear profilePicture error when a file is selected
  ...
  function removeProfilePicture(): void {
    ...
    setErrors((prevErrors) => ({ ...prevErrors, profilePicture: t("errors.profilePictureRequired") })); // Mark as required again
  ...
  const handleGoogleSignIn = () => {
    ...
    // Implement Google OAuth logic here (e.g., redirect to Google sign-in)
  };
      ...
      >
        {!(isSignUp && signUpStep === 2) && (
          <div className="relative lg:w-1/2 h-64 lg:h-auto">
            <Image
              src={"/image.png"}
              alt={isSignUp ? "House with magnifying glass" : "People with house"}
              fill
        ...
        <div
          className={`${
            !(isSignUp && signUpStep === 2) ? "lg:w-1/2" : "w-full"
        ...
        >
                ...
                {isSignUp ? (
                  <>
                    <span className="text-blue-600">
                      {/* Removed t("signUp.accountDetails") */}
                    </span>
                  </>
                ) : (
                  <>
              ...
              {isSignUp && (
                <p className="text-gray-600 mt-2">
                  ...
                  <span className="font-semibold">
                    {/* Removed the extra text from here as per design */}
                  </span>
                </p>
              )}
            </div>
          ...
          {isSignUp && signUpStep === 2 ? (
            ...
            <form className="space-y-6" onSubmit={handleAccountDetailsSubmit}>
              {/* Google Sign-In Button for Signup - Using Chrome icon from Lucide React */}
              <button
                type="button"
              ...
              >
                <Chrome size={20} /> {/* Using Chrome icon */}
              ...
              <div className="text-center text-sm text-gray-600 mt-6">
                ...
                <button
                  ...
                  onClick={() => {
                    setIsSignUp(false);
                    setIsSignUpPartner(false); // Reset partner state when switching to login
                  }}
                >
                  {t("login")}
                </button>
                ...
                <button
                  ...
                  onClick={() => {
                    setIsSignUp(true);
                    setIsSignUpPartner(true); // Set partner state when clicking Signup Partner
                  }}
                >
                  {t("signupPartner")}
                </button>
            ...
            <form className="space-y-6" onSubmit={handleSignInSubmit}>
              {/* Google Sign-In Button for Signin - Using Chrome icon from Lucide React */}
              <button
                type="button"
                onClick={handleGoogleSignIn}
                className="w-full flex items-center justify-center space-x-2 border border-gray-300 rounded-lg py-3 text-gray-700 font-semibold hover:bg-gray-50 transition-colors"
              ...
              <div className="text-center text-sm text-gray-600 mt-6">
                ...
                <button
                  ...
                  onClick={() => {
                    setIsSignUp(true);
                    setIsSignUpPartner(false); // Ensure this is false for regular signup
                  }}
                >
