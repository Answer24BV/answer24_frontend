...
export function AvatarCard({ avatar, userPlan }: AvatarCardProps) {
    ...
    <AvatarModal avatar={avatar} userPlan={userPlan}>
      ...
      >
        ...
        <CardContent className="pb-6 flex flex-col items-cente w-full">
          ...
          <Badge className={cn("px-3 py-1 rounded-full text-xs font-medium", planBadgeColor[avatar.requiredPlan])}>
            {avatar.requiredPlan.charAt(0).toUpperCase() + avatar.requiredPlan.slice(1)} Plan
          </Badge>
        </CardContent>
      </Card>
    </AvatarModal>
