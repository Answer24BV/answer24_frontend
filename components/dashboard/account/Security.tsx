...
async function getActiveSessionsAPI(): Promise<ApiResult<any[]>> {
  try {
    const res = await fetchWithAuth("/active-sessions", { method: "GET" });
    const json = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(json.message || "Failed to fetch active sessions");
    // Accept a couple of shapes: {data: []} or [] or {sessions: []}
...
async function revokeSessionAPI(sessionId: string): Promise<ApiResult> {
  try {
    const res = await fetchWithAuth(`/active-sessions/${sessionId}`, {
      method: "DELETE",
    });
    const json = await res.json().catch(() => ({}));
          ...
          <ul className="mt-4 space-y-4">
            {sessions.length === 0 ? (
              ...
              sessions.map((session: any) => {
                const label = session.is_current_session ? "Current session" : session.last_active_at ? `Last active: ${formatRelativeTime(session.last_active_at)}` : "Last active: —";
                return (
                  <li key={session.id} className="flex items-center justify-between">
                    <div>
                      <p className="font-medium">{session.user_agent || "Unknown device"}</p>
                      <p className="text-sm text-gray-500">{session.ip_address || "—"} — {label}</p>
                    </div>
                    <Button variant="outline" onClick={() => handleRevokeSession(session.id)} disabled={revokingId === session.id}>
