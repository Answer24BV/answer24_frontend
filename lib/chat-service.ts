...
export class ChatService {
  ...
  async createHelpdeskChat(): Promise<Chat> {
    try {
      ...
      console.log("ChatService.createHelpdeskChat: Response status:", response.status)
      console.log("ChatService.createHelpdeskChat: Response headers:", Object.fromEntries(response.headers.entries()))

      if (!response.ok) {
        const errorText = await response.text()
        console.error("ChatService.createHelpdeskChat: Error response:", errorText)
...
export const sendMessage = (chatId: string, content: string, senderId: string, type?: "text" | "image" | "file", attachments?: File[]) => 
  chatService.sendMessage(chatId, content, senderId, type, attachments)
export const createChat = (participantIds: string[], title?: string) => chatService.createChat(participantIds, title)
export const createHelpdeskChat = () => chatService.createHelpdeskChat()
export const generateAIResponse = (chatId: string, message: string) => chatService.generateAIResponse(chatId, message)
export const toggleAIForChat = (chatId: string, enabled: boolean) => chatService.toggleAIForChat(chatId, enabled)
export const markMessageAsRead = (messageId: string) => chatService.markMessageAsRead(messageId)
